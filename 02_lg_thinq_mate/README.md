...
## 배경 / 목표 / 성공 기준
- **배경**: ThinQ는 경쟁 서비스 대비 설치율은 높지만 실사용률이 저조하다.
- **미션(주어진 과제)**: "어떻게 하면 고객이 LG와 하루에 한 번은 만날 수 있을까?"
- **팀이 재정의한 문제**: 원인은 "중장년층의 디지털 취약"이 아니라 "기능 복잡성 + 정서적 니즈 미충족"이다.
- **성공 기준**: 위 재정의를 3개 독립 데이터 소스(커뮤니티·유튜브 댓글·앱 리뷰)로 교차 검증해 뒷받침한다.
...
- **방법**: Selenium으로 무한 스크롤 및 '더보기' 버튼을 제어해 댓글 전수 수집(하루 만에 **99건**) → 키워드 딕셔너리 기반 if-elif 구조로 5개 유형(오류 문의/사용법 질문/호환성 문의/개선요구/긍정후기) + 기타로 분류 → 파이차트 시각화
...
## 👤 본인 역할
- 82쿡·유튜브 댓글 분석은 직접 구현, 플레이스토어 리뷰 분석은 팀원 구현 과정을 지속적으로 논의·지도하며 최종 코드 정리 담당
- 통계청 자료로 팀 내 근거 없는 가설을 반박, 문제 재정의 주도
- 크롤링 설계(무한스크롤 제어, 예외처리)
- 위 3가지 분석 결과를 바탕으로 최종 솔루션(숏폼 기반 기능 가이드 'ThinQ 찰나') 아이디어 제안 및 솔루션명 'LG ThinQ 메이트' 명명
- 최종 발표 PPT의 Appendix(데이터 분석 파트) 작성

## 🏆 심사 결과
LG전자 DX School 6기 BX 프로젝트, **10팀 참가 중 우수상(2위)**.
심사표 기준 총점 85점(100점 만점). 문제 해석·타겟 고객 정의·데이터 확보 전략 항목에서 고득점을 받았으며, 데이터 시각화 효과성과 솔루션 구체성 항목은 상대적으로 보완이 필요하다는 평가를 받았습니다.
...
## 🔎 한계 / 아쉬운 점
- 유튜브 댓글 유형 분류를 키워드 사전 기반 if-elif 구조로 처리해, 사전에 없는 표현은 '기타'로 밀려났다. 분류 정확도를 사람이 표본 검수해 확인하지 않았다.
- 5060 세대를 하나의 동질 집단으로 취급했으나, 실제로는 디지털 활용 역량 편차가 클 수 있다.
- 82쿡·우아한 갱년기는 특정 성향의 이용자가 모인 커뮤니티라 표본 편향 가능성이 있다.
- 데이터 시각화가 분석 결과를 효과적으로 전달하지 못했다는 심사 피드백을 받았다.

## 🔁 다시 한다면
- 댓글 200건을 무작위 추출해 수동 라벨링과 자동 분류의 일치율을 먼저 측정하고, 그 수치를 함께 제시하겠다.
- 숏폼 가이드와 커뮤니티 중 어느 기능이 문제 해결에 기여하는지 분리 검증할 수 있도록 A/B 설계를 미리 준비하겠다.
- 분석 결과를 더 직관적으로 전달할 수 있는 시각화 방식을 미리 설계하겠다.
